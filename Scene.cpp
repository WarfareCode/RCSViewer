...
osg::Node* Scene::loadIslands(const std::string& terrain_shader_basename)
{
	...
	osg::BoundingSphere boundIslands = island->getBound();

	osg::ref_ptr<osg::Node> boat = osgDB::readNodeFile("D:/osg3.2.0/taiwan/iso.ive");
	//osg::ref_ptr<osg::Node> boat = osgDB::readNodeFile("glider.osg");
	osg::PositionAttitudeTransform* pat1 = new osg::PositionAttitudeTransform;
	pat1->addChild(boat);

	pat1->setScale(osg::Vec3(100.0, 100.0, 100.0));
	osg::BoundingSphere boud = pat1->getBound();

	osg::Vec3d vecPos = /*boundIslands.center()*/ - boud.center();
	vecPos.z() = 0.0;

	pat1->setPosition(vecPos);
	...
	island->setNodeMask(_oceanScene->getNormalSceneMask() |
                         ...
                         _oceanScene->getHeightmapMask()      | 
                         RECEIVE_SHADOW);
//	pat1->getStateSet()->addUniform(new osg::Uniform("uTextureMap", 0));
